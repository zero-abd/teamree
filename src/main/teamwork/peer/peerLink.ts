...
export function createPeerLink(options: PeerLinkOptions): PeerLink {
  ...
  const snapshot = (): PeerLinkStatus => {
    ...
    // `LINK_QUIET_AFTER_MS`. The transport's own stamp, which is the one the
    // deadline reads, so the window and the teardown quote one number.
    if (quiet && transport) status.lastHeardAt = transport.lastDecryptedAt
    return status
  }
  ...
  const armQuietWatch = (delayMs: number): void => {
    cancelQuietWatch = options.scheduler.setTimer(
      () => {
        cancelQuietWatch = undefined
        const active = transport
        if (!active) return
  ...
  const establish = (): void => {
    ...
    // is fatal. The transport stamps itself as heard-from when it is built, so
    // this starts from the moment the session did.
    ...
    connection = openRelayConnection({
      ...
      events: {
        ...
        onBinary: (payload) => {
          ...
          if (quiet && transport && options.scheduler.now() - transport.lastDecryptedAt < LINK_QUIET_AFTER_MS) {
            quiet = false
            options.onStatusChange(snapshot())
            armQuietWatch(LINK_QUIET_AFTER_MS)
          }
