// TEMPORARY. A probe, not a test: it asserts almost nothing and prints what the
// platform actually did, so the macOS runner can be read as an instrument.
// Delete once the answer is in the watcher.
...
describe('macOS watch probe', () => {
  it('says where it is running', async () => {
  ...
  it('probe 0: does a bare fs.watch on one directory ever fire', async () => {
  ...
  it('probe A: attach then write immediately, watched for 20s', async () => {
  ...
  it('probe B: attach, wait 1s, then write', async () => {
    for (let trial = 1; trial <= 5; trial += 1) {
      const root = await checkout()
      await writeMember(root, 'ana')
      const targets = [root, join(root, '.teamree'), join(root, '.teamree', 'members')]
  ...
  it('probe C: arming latency, poked every 25ms until it speaks', async () => {
    for (const persistent of [false, true]) {
      const armed: number[] = []
      for (let trial = 1; trial <= 8; trial += 1) {
        ...
        armed.push(rig.seen.length === 0 ? -1 : Math.round(rig.seen[0].at))
        rig.close()
      }
  ...
  it('probe D: what the checkout-root watch is told', async () => {
    const root = await checkout()
    await writeMember(root, 'ana')
