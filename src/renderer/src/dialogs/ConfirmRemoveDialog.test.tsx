...
// Removing a worktree: the question names it, lists what would go, and never shows the runtime's own words.
...
describe('what it asks', () => {
  it('names the worktree as the sidebar does, with the path only in the title’s tooltip', async () => {
    await mount()
    const dialog = screen.getByRole('dialog', { name: 'Remove "Rewrite the pager"?' })
  ...
  it('still asks when the worktree itself has already gone', async () => {
    seed({ worktrees: [] })
    await mount()
    expect(screen.getByRole('dialog', { name: 'Remove this worktree?' })).toBeTruthy()
  })
})

describe('the answers', () => {
  it('offers Cancel, focused, and Remove in red', async () => {
    await mount()
    const buttons = [...document.querySelectorAll('.modal__actions .button')]
    expect(buttons.map((button) => button.textContent)).toEqual(['Cancel', 'Remove'])
