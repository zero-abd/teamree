...
The hero clip and the feature clips under `site/public/demos/` other than
`waiting-on-you` were retaken from a `main` build after 0.2.0, in a hidden window
on a throwaway profile against a small demo repository; `waiting-on-you` and the
`screenshot.png` fallback are still 0.2.0's. The `cli` and `teamwork` clips still do
